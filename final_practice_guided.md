...
In this task, you'll implement a Graph-based QA agent that reads a user story from a local file via MCP, generates test cases, passes them through an automated quality gate, walks through a human-in-the-loop review, and writes the final output to a local file.
...
├── mcp_config.json     ← provided: filesystem MCP server config (Task 2)
...
**MCP (Model Context Protocol)** — a standard for connecting LLMs to external context sources. You will use `@modelcontextprotocol/server-filesystem` via Node.js to give Claude access to local files without writing custom file I/O wrappers.
...
The filesystem MCP server is already configured to serve the current working directory (`.`). No credentials are required:
...
Create a plain-text file (e.g. `user_story.txt`) in the project root that contains your user story. This is the file the Context Agent will read. Example:
...
| **Field** | **Type** | **Purpose** |
| --- | --- | --- |
| `file_path` | `str` | Path to the input file containing the user story |
| `user_story` | `str` | Raw text of the user story read from the file |
...
| `published` | `bool` | `True` after the test cases are written to a local file |
